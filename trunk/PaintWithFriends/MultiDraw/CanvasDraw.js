function CanvasDraw(target, options) {
    ...
    function ExtendPolylineTo(wrapper, id, x, y) {
        ...
        var event = new CustomEvent('move', { "detail": { x: x, y: y, color: foregroundPolylines[id].color } });
    ...
    function DrawPolyline(ctx, polyline) {
        if (polyline.points.length == 1) {
            ctx.beginPath();
            ctx.arc(polyline.points[0].x, polyline.points[0].y, 5, 0, Math.PI, true);
            ctx.arc(polyline.points[0].x, polyline.points[0].y, 5, Math.PI, Math.PI * 2, true);
...
/// Polyfill CustomEvent for IE9+10
(function () {
